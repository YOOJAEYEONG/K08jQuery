...
$(function(){
    /*
    문서의 로드가 끝난 직후 js파일의 내용을 읽어와서
    현재 문서에 로드한다. 즉, $.load()메소드와 동일한 역할을 하고있다.
    
    .load() : 서버에서 데이터를 읽은 후, 읽어 들인 HTML 코드를 선택한 요소에 배치함.
        사용법: $("#box").load("test.txt #para");
        =>test.txt 파일 내에서 아이디가 para인 요소만을 읽어 들여,
        아이디가 box인 요소 안에 배치하는 예입니다.
        load() 메소드의 인수로 URL 주소와 함께 선택자를 전달할 때는
        하나의 문자열로 전송해야 합니다.
